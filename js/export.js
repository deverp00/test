...
function getFilteredData(module) {
  switch (module) {
    ...
    case 'fees': {
      ...
      const sectionFilter = document.getElementById('feeSectionFilter')?.value || 'all'; // not used in latest fee version but kept for compatibility
      ...
      // Section filter removed in latest fee UI, but we keep it for backward compatibility
