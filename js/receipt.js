...
function showReceipt(id) {
  ...
  const payment = window.PAYMENTS.find(p => p.receiptNo === fee.receiptNo);
...
function showSalaryReceipt(id) {
  ...
  // Generate receipt number if not stored
  const receiptNo = `SAL-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`;
  // Use payment date from record if available, else generate
